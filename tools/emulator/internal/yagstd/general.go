...
var ErrStringTooLong = errors.New("String is too long (max 1MB)")
...
func slice(item reflect.Value, indices ...reflect.Value) (reflect.Value, error) {
	...
	case reflect.Array, reflect.Slice, reflect.String:
		...
		case 2:
			...
			endIndex = args[1]
		default:
			return reflect.Value{}, fmt.Errorf("unexpected slice arguments %d", len(args))
		}

		if startIndex < 0 || startIndex >= v.Len() {
			return reflect.Value{}, fmt.Errorf("start index out of range: %d", startIndex)
		} else if endIndex <= startIndex || endIndex > v.Len() {
			return reflect.Value{}, fmt.Errorf("end index out of range: %d", endIndex)
		}
